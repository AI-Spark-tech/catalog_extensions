...
def get_product_filter_data_with_price(query_args=None):
    ...
    try:
        ...
        # DUPLICATE FIX: Remove duplicate items caused by child table joins
        # When filtering by offers/badges, ProductQuery may return duplicates
        # if an item has multiple offer/badge rows.
        if result.get("items"):
            seen = set()
            unique_items = []
            ...
            # Update result with deduplicated items
            result["items"] = unique_items
            # Update count to match deduplicated list
