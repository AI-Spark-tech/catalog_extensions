frappe.ready(function () {
  ...
  function injectProductOffers() {
    ...
    var unprocessedCards = Array.prototype.filter.call(cards, function (card) {
      return !card.hasAttribute('data-offers-checked');
    });
    ...
    function applyOffers() {
      unprocessedCards.forEach(function (card) {
        ...
        if (!code) return;
        var offers = offersCache[code];

        // Mark as checked regardless (prevents repeated API calls if no offers)
        card.setAttribute('data-offers-checked', '1');
        ...
        offers.forEach(function (o) {
          ...
          line.className = 'offer-line mt-2 d-flex';
          line.innerHTML =
            '<div class="offer-line-icon">' +
            '<svg width="16" height="16" viewBox="0 0 24 24" stroke="currentColor" fill="none" xmlns="http://www.w3.org/2000/svg">' +
            ...
            '<path d="M15.5 14.5C15.5 15.0523 15.0523 15.5 14.5 15.5C13.9477 15.5 13.5 15.0523 13.5 14.5C13.5 13.9477 13.9477 13.5 14.5 13.5C15.0523 13.5 15.5 8.94772 15.5 14.5Z" fill="white" stroke-linecap="round" stroke-linejoin="round"/>' +
            '</svg>' +
            '</div>' +
            '<p class="offer-line-text mb-0">' +
    ...
    var missingCodes = itemCodes.filter(function (c) { return !offersCache[c]; });
      ...
      callback: function (r) {
        ...
        if (r && r.message) {
          Object.keys(r.message).forEach(function (code) {
            offersCache[code] = r.message[code];
          });
        }
        // Ensure missing codes are still cached to avoid repeated lookups
        missingCodes.forEach(function (c) {
          if (!offersCache[c]) offersCache[c] = [];
        });
  ...
  document.addEventListener('products-loaded', function () {
    setTimeout(injectProductOffers, 50);
  });
  ...
  function setupOffersObserver() {
    ...
    var observer = new MutationObserver(function () {
      ...
      timeoutId = setTimeout(injectProductOffers, 600);
    });
