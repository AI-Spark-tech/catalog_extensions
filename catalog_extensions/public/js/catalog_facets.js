...
function setupMobileOffCanvasFilters() {
  // Create Amazon-style off-canvas mobile filter UI
  ...
  // Clone original filters content into off-canvas sidebar body so
  // desktop sidebar remains populated. Use a new id on the clone to
  // avoid duplicate element ids in the DOM.
  const sidebarBody = sidebar.querySelector(".ce-mobile-filters-body");
  const filtersClone = filtersContainer.cloneNode(true);
  filtersClone.id = "product-filters-mobile";
  // Remove desktop-only / layout classes so cloned filters are visible in
  // the mobile off-canvas and don't inherit collapse / d-md-block behavior.
  filtersClone.classList.remove("collapse", "d-md-block", "mr-4");
  filtersClone.classList.add("offcanvas-filters");
  sidebarBody.appendChild(filtersClone);
