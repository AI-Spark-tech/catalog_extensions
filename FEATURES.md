...
| Checkout settings UI controls | Optionally hides shipping and payment UI on the cart page and aligns order-page actions directly from the checkout visibility flags. | `catalog_extensions/public/js/simple_checkout.js`, `catalog_extensions/simple_checkout.py` |
...
| Checkout settings backend | Applies default address and payment-term behavior, gates cancel/return/refund visibility, and preserves core checkout flow. | `catalog_extensions/simple_checkout.py`, `catalog_extensions/api.py` |
...
| Webshop Checkout Settings DocType | Lets admins configure shipping/payment visibility defaults and manage cancel-order availability without a separate master enable switch. | `deploy/setup_doctypes.py`, `catalog_extensions/public/js/webshop_simple_checkout_settings.js` |
...
| `Webshop Simple Checkout Settings` | Site-level settings powering the admin-facing "Webshop Checkout Settings" experience |
